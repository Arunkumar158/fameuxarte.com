...
const ArtworkDetails = () => {
  ...
  const structuredData = generateProductStructuredData({
    name: artwork.title,
    description: artwork.description || `${artwork.title} by ${artwork.artist?.name || 'Unknown Artist'}`,
  ...
  return (
    <MainLayout>
      <SEO
        title={`${artwork.title} | Gallery Canvas Commerce`}
        description={artwork.description || `${artwork.title} by ${artwork.artist?.name || 'Unknown Artist'}`}
        canonicalUrl={`/artworks/${artwork.id}`}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="relative aspect-square rounded-lg overflow-hidden bg-muted">
            <img
              ...
              alt={`${artwork.title} by ${artwork.artist?.name || 'Unknown Artist'}`}
