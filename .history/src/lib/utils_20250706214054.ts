...
export const extractRelativePath = (imagePath: string): string => {
  ...
  try {
    ...
    // Example: https://oqslvwynlppuacdrhlxl.supabase.co/storage/v1/object/sign/artworks/folder/image.png
    const pathSegments = url.pathname.split('/').filter(segment => segment !== '');
    const storageIndex = pathSegments.findIndex(segment => segment === 'storage');
    
    if (storageIndex !== -1) {
      // Look for 'artworks' after the storage path
      const artworksIndex = pathSegments.findIndex((segment, index) => 
        index > storageIndex && segment === 'artworks'
      );
