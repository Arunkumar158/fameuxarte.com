...
/**
 * Get a direct image URL (for components that need immediate access)
 * This is a simplified version that doesn't use signed URLs
 * @param imagePath - The image path from the database
 * @returns string - The direct URL or placeholder
 */
export const getDirectImageUrl = (imagePath: string | null): string => {
  ...
  try {
    ...
    // For now, return a placeholder since we can't use async operations here
    // Components should use useArtworkImage hook for proper URL handling
    return '/placeholder.svg';
  } catch (err) {
    console.error("❌ Error getting direct image URL:", err);
    return '/placeholder.svg';
  }
};
